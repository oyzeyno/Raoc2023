#' Extraction of engine part ids (numeric) which are ortogonally or diagonally
#' adjacent to symbols except "."
#'
#' @param engine_schema a character matrix of "engine schema"
#' @param all_part_ids a data frame of all part ids with row information
#'
#' @return a list of part ids that are adjacent to symbols
#' @export
#'
extract_engine_part_id <- function(engine_schema,all_part_ids){

  # Determine which characters to search for
  tt<- as.character(engine_matrix)
  search_for_these <- setdiff(tt, c(".",0:9))
  digits <- c(0:9)
  part_ids <- c()
